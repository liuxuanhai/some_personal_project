	...
	<div class="header-top">
 	...
 	<div class="content">
		...
		<div class="form-inline" style="padding:10px;">
			...
			<input class="hide" id="hidetypeid" value="${sessionScope.typeid}"/>
		...
		<c:forEach items="${sessionScope.goodinfolist}" var="goodinfo">
			<div class="row" style="padding:15px;border:1px solid #EEE">
			<div class="col-md-5" style="height:200px">
				<c:set var="imagelist" value="${fn:split(goodinfo.goodimage, '#')}" />
				<a href="goodinfo.do?method=goodinfoLookExecute&goodid=<c:out value="${goodinfo.goodid}"/>">
				<img  src="<%=path %>/upload/${imagelist[0]}" width="100%" height="200">
				</a>
			</div>
			<div class="col-md-4" style="height:200px">
				<p>产品名称：<c:out value="${goodinfo.goodname}"/></p>
				<p>产品类别：<c:out value="${goodinfo.typename}"/></p>
				<p>产品描述：<c:out value="${goodinfo.gooddescr}"/></p>
			</div>
			<div class="col-md-2" style="height:200px">
			¥<span style="font-size:25px;color:#58E"><c:out value="${goodinfo.goodprice}"/></span>
			</div>
			</div>
...
function onfindinfo()
{
	window.location.href="goodinfo.do?method=goodinfoListExecute&findinfo="+encodeURI(encodeURI($("#findinfo").val()))+"&typeid="+encodeURI(encodeURI($("#typeid").val()));	
}
$(function () {
	$("#typeid").val($("#hidetypeid").val());
});
</script>
