		...
		<div class="container">
		  ...
		  <div class=" h_menu4">
				<ul class="memenu skyblue">
				    ...
				    <li class="active grid"><a class="color2" href="hotel.do?method=hotelListExecute">酒店预订</a>
			    	</li>
		...
		<div style="padding:10px;">
			...
			<div class="col-md-7" style="height:200px">
				   ...
				   <div class="carousel-inner text-center" style="height:280px;">
				      <c:set var="imagelist" value="${fn:split(sessionScope.hotelinfo.hotelimage, '#')}" />
			...
			<div class="col-md-5" style="height:280px">
				<h2 style="color:#58E" id="">${sessionScope.hotelinfo.hotelname}</h2>
				<br/><br/>
				<p>所在城市：${sessionScope.hotelinfo.hotelcity}</p>
				<p>详细地址：${sessionScope.hotelinfo.hoteladdr}</p>
				<p>最低价格：¥<span style="font-size:20px;color:#E87">${sessionScope.hotelinfo.hotelprice}</span></p>
			</div>
			</div>
				...
				<div class="cd-tabs">
						<nav>
							<ul class="cd-tabs-navigation">
								<li><a data-content="roomtype"  href="#0" class="selected ">房型预订</a></li>
								<li><a data-content="description"  href="#0" >酒店描述</a></li>
								<li><a data-content="spotcity" href="#0" >同城景点</a></li>
								<li><a data-content="comment" href="#0" >相关评论</a></li>
							</ul> 
						</nav>
						<ul class="cd-tabs-content">
							<li data-content="roomtype" class="selected ">
								<div class="facts">
							      	<table class="table table-bordered table-hover definewidth m10">
							      		<tr>
							      		<th>房型类型</th><th>房型描述</th><th>预订单价(元/天)</th><th>预订</th>
							      		</tr>
							      		<c:forEach items="${sessionScope.hotelroomtype}" var="hotelroomtype">
							      		<tr>
							      			<td class="hide"><c:out value="${hotelroomtype.roomid}"/></td>
							      			<td ><c:out value="${hotelroomtype.roomname}"/></td>
							      			<td ><c:out value="${hotelroomtype.roomdescr}"/></td>
							      			<td >
							      			<c:out value="${hotelroomtype.roomprice}"/>
							      			</td>
							      			<td> 
							      			<a href="javascript:viod(0)" onclick="ongotoyuding(this)" class="btn btn-success">预订</a>
							      			</td>
							      		</tr>
							      		</c:forEach>
							      	</table>
							...
							<li data-content="description">
							<div class="facts">
							<h4>景点描述</h4>
							  <p>
							  	${sessionScope.hotelinfo.hoteldescr}
							  </p>
							<h4>备注信息</h4>
							  <p>
							  	${sessionScope.hotelinfo.hotelremark}
							  </p>	      
					        </div>
								...
								<c:forEach items="${sessionScope.viewspotcity}" var="viewspotcity">
									<div class="row" style="padding:10px;">
										<div class="col-md-5" style="height:80px">
											<c:set var="imagelist" value="${fn:split(viewspotcity.sportimage, '#')}" />
											<a href="viewspot.do?method=spotviewLookExecute&spotid=<c:out value="${viewspotcity.spotid}"/>">
											<img  src="<%=path %>/upload/${imagelist[0]}" width="100%" height="200">
											</a>
										</div>
										<div class="col-md-4" style="height:80px">
											<p>景点名称：<c:out value="${viewspotcity.spotname}"/></p>
											<p>所在城市：<c:out value="${viewspotcity.spotcity}"/></p>
											<p>详细地址：<c:out value="${viewspotcity.spotaddr}"/></p>
										</div>
										<div class="col-md-2" style="height:200px">
										¥<span style="font-size:25px;color:#58E"><c:out value="${viewspotcity.spotprice}"/></span>
										</div>
									</div>
								</c:forEach>		        
						    </div>
						...
						<li data-content="comment" >
							<c:forEach items="${sessionScope.hotelcomment}" var="hotelcomment">
								<div class="comments-top-top">
									<div class="top-comment-right">
										<h6><c:out value="${hotelcomment.username}"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<c:out value="${hotelcomment.addtime}"/></h6>
										<p><c:out value="${hotelcomment.content}"/></p>
									</div>
								</div>
							</c:forEach>
						</li>
  ...
  <div class="modal-dialog">
    <form id="addform" class="form" action="#" method="post">
    	<div class="modal-content message_align">
	      ...
	      <div class="modal-body">
	      <input class="hide" id="orderhotelid" value="${sessionScope.hotelinfo.hotelid}"/>
	      <div class="m10">
	      	酒店名称：<span id="orderhotelname" style="font-size:25px">${sessionScope.hotelinfo.hotelname}</span>
	      </div>
	      <div class="m10">
	      <input class="hide" id="orderroomid" value=""/>
	      	预订房型：<span id="orderroomname"></span>
	      </div>
	      <div class="m10">
	      
	      	预订单价：¥<span style="font-size:20px;color:#E87" id="orderroomprice"></span>
	      </div>
	      ...
	      <div class="m10">
	      	预订总额：¥<span style="font-size:20px;color:#E87" id="ordertotalprices"></span>
	      </div>
	      <div class="m10 form-inline">
	      	&nbsp;&nbsp;&nbsp;&nbsp;联系人：
	      ...
	      <div class="m10 form-inline">
	      	入住日期：<input type="text" id="ordercheckintime" readonly class="form_datetime form-control"/>
	      </div>
	      </div>
	      <div class="modal-footer text-center">
	      	 <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>&nbsp;&nbsp;
...
function ongotoyuding(obj)
{
	...
	$("#orderroomid").val(obj.parentNode.parentNode.cells[0].innerHTML);
	$("#orderroomname").html(obj.parentNode.parentNode.cells[1].innerHTML);
	$("#orderroomprice").html(obj.parentNode.parentNode.cells[3].innerHTML);
	$("#ordertotalprices").html(obj.parentNode.parentNode.cells[3].innerHTML);
	$("#ordernums").val("1");
...
{
	$("#ordertotalprices").html(parseFloat($("#orderroomprice").html())*parseFloat($("#ordernums").val()));	
}
...
function ordercheck()
{
	...
	if($("#ordername").val()=="")
	{
		alert("请输入入住联系人姓名");return;
	}
	if($("#orderphone").val()=="")
	{
		alert("请输入入住联系电话");return;
	}
	if($("#ordercheckintime").val()=="")
	{
		alert("请输入入住日期");return;
	}
	//开始提交订单
	var dataarray=new Array();
	dataarray.push($("#orderhotelid").val());
	dataarray.push($("#orderhotelname").html()); // 酒店
	dataarray.push($("#orderroomid").val());  //房型id
	dataarray.push($("#orderroomname").html());  //房型
	dataarray.push($("#orderroomprice").html());
	dataarray.push($("#spotusername").val());  //用户名
	...
	dataarray.push($("#ordercheckintime").val());
	spotuser.hotelorderAdd(dataarray,hotelordercallback);
}

function hotelordercallback()
{
