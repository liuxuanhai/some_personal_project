 	...
 	<div class="content">
		...
		<c:forEach items="${sessionScope.hotellist}" var="hotel">
			...
			<div class="col-md-5" style="height:200px">
				<c:set var="hotelimagelist" value="${fn:split(hotel.hotelimage, '#')}" />
				<a href="hotel.do?method=hotelLookExecute&hotelid=<c:out value="${hotel.hotelid}"/>">
				<img  src="<%=path %>/upload/${hotelimagelist[0]}" width="100%" height="200">
				</a>
			</div>
			<div class="col-md-4" style="height:200px">
				<p>酒店名称：<c:out value="${hotel.hotelname}"/></p>
				<p>所在城市：<c:out value="${hotel.hotelcity}"/></p>
				<p>详细地址：<c:out value="${hotel.hoteladdr}"/></p>
			</div>
			<div class="col-md-2" style="height:200px">
			¥<span style="font-size:25px;color:#58E"><c:out value="${hotel.hotelprice}"/></span>
			</div>
			</div>
...
function onfindinfo()
{
	window.location.href="hotel.do?method=hotelListExecute&findinfo="+encodeURI(encodeURI($("#findinfo").val()))+"&city="+encodeURI(encodeURI($("#city").val()));	
}
$(function () {
