...
%>
<%
String str="select ktname,lastlwpath from keti where stuid='"+request.getParameter("stuid").toString()+"'";
...
<h4>论文终稿提交状态：
     <%if(rs.getString("lastlwpath")!=null&&!rs.getString("lastlwpath").equals("")) {%>
		    已提交&nbsp;&nbsp;<a target="_blank" href="../pdfjs/web/viewer.html?file=<%=path %>/upload/<%=rs.getString("lastlwpath") %>">查看</a>&nbsp;&nbsp;<a href="../servlet/fileDownloadServlet?filepath=<%=rs.getString("lastlwpath") %>&stuid=<%=request.getParameter("stuid").toString() %>">下载</a>
		    <%} else{%>
		    未提交，请按学校时间要求及时上传论文终稿。
		    <%} %>
		    </h4>
</div>
...
<input type="hidden" value="lastlwpath" id="pathtype" name="pathtype"/>
<table class="table table-bordered table-hover definewidth m10">
   <tr>
    <td width="20%">论文终稿文件(仅支持pdf)</td>
    <td>
    <input type="file" id="file" name="file" accept="application/pdf"/>
    <a href="#" onclick="sendfile()">提交论文终稿</a>
    </td>
   </tr>
</table>
</form>
