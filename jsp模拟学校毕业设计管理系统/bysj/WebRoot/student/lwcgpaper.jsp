...
%>
<%
String str="select ktname,lwpath from keti where stuid='"+request.getParameter("stuid").toString()+"'";
...
<h4>论文草稿提交状态：
     <%if(rs.getString("lwpath")!=null&&!rs.getString("lwpath").equals("")) {%>
		    已提交&nbsp;&nbsp;<a target="_blank" href="../pdfjs/web/viewer.html?file=<%=path %>/upload/<%=rs.getString("lwpath") %>">查看</a>&nbsp;&nbsp;<a href="../servlet/fileDownloadServlet?filepath=<%=rs.getString("lwpath") %>&stuid=<%=request.getParameter("stuid").toString() %>">下载</a>
		    <%} else{%>
		    未提交，请按学校时间要求及时上传论文草稿。
		    <%} %>
		    </h4>
</div>
...
<input type="hidden" value="lwpath" id="pathtype" name="pathtype"/>
<table class="table table-bordered table-hover definewidth m10">
   <tr>
    <td width="20%">论文草稿报告文件(仅支持pdf)</td>
    <td>
    <input type="file" id="file" name="file" accept="application/pdf"/>
    <a href="#" onclick="sendfile()">提交论文草稿</a>
    </td>
   </tr>
</table>
</form>
