...
%>
<%
String str="select ktname,zqpath from keti where stuid='"+request.getParameter("stuid").toString()+"'";
...
<h4>中期报告状态：
     <%if(rs.getString("zqpath")!=null&&!rs.getString("zqpath").equals("")) {%>
		    已提交&nbsp;&nbsp;<a target="_blank" href="../pdfjs/web/viewer.html?file=<%=path %>/upload/<%=rs.getString("zqpath") %>">查看</a>&nbsp;&nbsp;<a href="../servlet/fileDownloadServlet?filepath=<%=rs.getString("zqpath") %>&stuid=<%=request.getParameter("stuid").toString() %>">下载</a>
		    <%} else{%>
		    未提交，请及时上传中期报告。
		    <%} %>
		    </h4>
...
<input type="hidden" value="zqpath" id="pathtype" name="pathtype"/>
<table class="table table-bordered table-hover definewidth m10">
   <tr>
    <td width="20%">中期报告文件(仅支持pdf)</td>
    <td>
    <input type="file" id="file" name="file" accept="application/pdf"/>
    <a href="#" onclick="sendfile()">提交中期报告</a>
    </td>
   </tr>
</table>
</form>
