...
<input type="hidden" value="${sessionScope.stuclassroom.teacher.teaid}" id="teaid"/>
...
<caption class="text-center">${sessionScope.stuclassroom.classname}</caption>
    ...
    <tr>
        ...
        <tr>
			...
			<td><c:out value="${student.classroom.academy.academyname}"/></td>
			...
			<td>
			<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<c:out value="${student.qqid}"/>&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:<c:out value="${student.qqid}"/>:51" alt="点击这里给我发消息" title="点击这里给我发消息"/></a>
		</td>
		</tr>
		</c:forEach>
</table>
...
function onfind()
{
	window.location.href="../student.do?method=studentAcademyFindByClassIDExecute1&classid="+$("#classid").val()+"&findinfo="+encodeURI(encodeURI($("#findinfo").val()));
}
function onback()
{
	window.location.href="../teacher.do?method=getTeacherClassExecute&teacherid="+$("#teaid").val();
}
function onprevious()
{
	...
	window.location.href="../student.do?method=studentAcademyFindByClassIDExecute1&classid="+$("#classid").val()+"&findinfo="+encodeURI(encodeURI($("#findinfo").val()))+"&pagenumber="+pagenumber;
}
function onnext()
{
